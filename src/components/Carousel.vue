  ...
  methods: {
    ...
    approveClicked: function() {
      let properties = Object.getOwnPropertyNames(
        this.container_images[this.slide]
      );
    ...
    rejectClicked: function() {
      burgersRef
        ...
        .update({ was_reviewed: true, is_validated: true });
    },
    setClicked: function() {
      console.log(this.images[this.slide]);
      burgersRef
        .child(this.image_key)
        .update({ highlight_image: this.images[this.slide] });
    }
  },
  mounted() {
    if (this.type == "thumbnail") {
      ...
      this.container_images.forEach(element => {
        if (element[".key"] == this.image_key) {
          this.approved_images_container = images_container
            .orderByKey()
